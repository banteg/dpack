...
def test_builder(pack, pack_type):
    builder = dpack.builder(network="ropsten")
    if pack_type:
        builder.pack_type(typename="WETH9", artifact="tests/weth-ropsten-artifact.json")
    for name in ["weth", "weth9"]:
        builder.pack_object(
            typename="WETH9",
            artifact="tests/weth-ropsten-artifact.json",
            objectname=name,
            address="0xc778417E063141139Fce010982780140Aa0cD5Ab",
        )
    assert builder.build().model_dump() == pack.model_dump()


def test_builder_chaining(pack):
    built = (
        dpack.builder(network="ropsten")
        .pack_type(typename="WETH9", artifact="tests/weth-ropsten-artifact.json")
        .pack_object(
            typename="WETH9",
            artifact="tests/weth-ropsten-artifact.json",
            objectname="weth",
